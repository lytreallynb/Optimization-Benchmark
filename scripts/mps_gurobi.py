...
详细中文注释:
本脚本是专门针对MPS格式文件设计的求解和报告生成工具，使用了业界领先的
Gurobi优化求解器作为后端。MPS是一种通用的数学规划问题文件格式，广泛用于
线性规划、整数规划等优化问题的描述。脚本不仅可以高效地求解这些问题，
还能生成格式精美的LaTeX报告，详细展示模型结构和求解结果。
通过智能变量格式化和排序功能，报告的可读性得到了显著提升。
"""
...
class MPSSolver:
    ...
    这个类是处理MPS文件的核心组件，利用Gurobi优化引擎实现:
    1. 读取MPS格式的数学规划模型
    2. 调用Gurobi求解器求解模型
    3. 提取求解结果和模型信息
    4. 生成专业的LaTeX格式报告
    
    该类特别关注报告的排版质量和用户体验，实现了多项增强功能:
    - 智能变量格式化，根据变量数量自动调整显示格式
    - 合理的变量和约束排序，确保逻辑顺序
    - 自动换行处理长表达式，避免PDF排版问题
    - 详细的解决方案分析和可视化
    """
